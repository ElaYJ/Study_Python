'''
 # [이진탐색] 정렬되어 있는 자료구조에서 중앙값과의 크고 작음을 이용해서 데이터를 검색한다.
     ▶ 핵심은 정렬된 자료구조 내에서 검색이 이루어진다는 것이다.
'''
import random
...
searchData = random.randint(1, 11)#int(input('찾으려는 숫자 입력: '))
searchResultIdx = -1
...
searchData = random.randint(4, 88) #int(input('찾으려는 숫자 입력: '))
print(f'search number: {searchData}')
searchResultIdx = -1
...
만약 '8'을 검색하게 되면...
 ...
 mid_idx에 {(start + end) // 2 = 3} 값을 대입하므로 mid_idx 값 3도 변하지 않는다.
 결국 오른쪽으로 이동해야 할 start_idx값과 mid_idx값이 변하지 않아 무한루프에 빠지게 된다.
+staIdx: 3, +midIdx: 3(7), endIdx: 4
+staIdx: 3, +midIdx: 3(7), endIdx: 4
+staIdx: 3, +midIdx: 3(7), endIdx: 4
+staIdx: 3, +midIdx: 3(7), endIdx: 4
+staIdx: 3, +midIdx: 3(7), endIdx: 4
...
+staIdx: 3, +midIdx: 3(7), endIdx: 4
.................무한반복
만약 '15'를 검색하면, 15 > mid_val(11)
3>+staIdx: 6, +midIdx: 6(11),  endIdx: 7
3>+staIdx: 6, +midIdx: 6(11),  endIdx: 7
3>+staIdx: 6, +midIdx: 6(11),  endIdx: 7
3>+staIdx: 6, +midIdx: 6(11),  endIdx: 7
3>+staIdx: 6, +midIdx: 6(11),  endIdx: 7
3>+staIdx: 6, +midIdx: 6(11),  endIdx: 7
3>+staIdx: 6, +midIdx: 6(11),  endIdx: 7
만약 '19'를 검색하면, 19 > mid_val(17)
3>+staIdx: 7, +midIdx: 7(17),  endIdx: 8
3>+staIdx: 7, +midIdx: 7(17),  endIdx: 8
3>+staIdx: 7, +midIdx: 7(17),  endIdx: 8
3>+staIdx: 7, +midIdx: 7(17),  endIdx: 8
3>+staIdx: 7, +midIdx: 7(17),  endIdx: 8
3>+staIdx: 7, +midIdx: 7(17),  endIdx: 8
3>+staIdx: 7, +midIdx: 7(17),  endIdx: 8
3>+staIdx: 2, +midIdx: 2(5),  endIdx: 3
'''
...
import module_srch_binary as bins
import random
...
if __name__ == '__main__':
    ...
    if resultIdx == -1:
        print(f'search number({searchNum}) NOT found.!!!')
